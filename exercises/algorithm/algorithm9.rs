    ...
    pub fn next(&mut self) -> Option<T> {
        ...
        unsafe {
            ...
            while self.children_present(curr) {
                let lc = self.left_child_idx(curr);
                let rc = self.right_child_idx(curr);
                match (
                    (self.comparator)(&self.items[curr], &self.items[lc]),
                    (self.comparator)(&self.items[curr], &self.items[rc]),
                ) {
                    (true, true) => break,
                    (true, false) => {
                        Self::swap(mut_ptr, rc, curr);
                        curr = rc;
                    }
                    (false, true) => {
                        Self::swap(mut_ptr, lc, curr);
                        curr = lc;
                    }
                    (false, false) => {
